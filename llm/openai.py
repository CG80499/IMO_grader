...
class OpenAI(BaseLLM):
    ...
    def total_cost(self) -> float:
        if "gpt-3.5-turbo" in self.model:
            ...
            return (
                2.0 * self.total_prompt_tokens / 1000**2
                + 8.0 * self.total_completion_tokens / 1000**2
            )
        else:
...
class OpenAIReasoning(BaseLLM):
    ...
    def total_cost(self) -> float:
        if "o1-preview" in self.model:
            ...
            return (
                4.00 * self.total_prompt_tokens / 1000**2
                + 16.00 * self.total_completion_tokens / 1000**2
            )
        else:
            raise ValueError(f"Cost tracking is not supported for model {self.model}")
